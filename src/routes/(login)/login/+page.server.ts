...
import {
	createNewUser,
	getUserByEmail,
} from "$lib/server/database/user.model.js";
...
export const actions = {
	login_with_email: async ({ request, getClientAddress }) => {
		...
		const ratelimit = env.SIGNIN_IP_RATELIMIT
			? Number.parseInt(env.SIGNIN_IP_RATELIMIT)
			: 20;
		...
		const verification_token = await createEmailVerificationToken(
			user.id,
			user.email,
		);
		const origin = new URL(request.url).origin;
		const verificationLink = `${origin}/login/email-verification?verification_token=${verification_token}`;
		...
		if (process.env.NODE_ENV === "test") {
			...
			writeFileSync(
				join(testResultsDir, "verification-link.txt"),
				verificationLink,
			);
		}
